...
RenderOptions parse_integrator(pugi::xml_node node,
    ...
    } else if (type == "manylight") {
        options.integrator = Integrator::ManyLight;
    } else if (type == "restir") {
        options.integrator = Integrator::ReSTIR;
        for (auto child : node.children()) {
            std::string name = child.attribute("name").value();
            if (name == "spatial_iteration") {
                options.sp_reuse_iterations = parse_integer(
                    child.attribute("value").value(), default_map);
            } else if (name == "reservior_size") {
                options.rsvr_size = parse_integer(
                    child.attribute("value").value(), default_map);
            } else if (name == "neighour_select") {
                options.nbr_slt = parse_integer(
                    child.attribute("value").value(), default_map);
            }
        }
    } else {
